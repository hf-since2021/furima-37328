...
RSpec.describe User, type: :model do
  ...
  describe '新規登録/ユーザー情報' do
    ...
    it 'パスワードは半角英数字混合での入力が必須（数字だけでは登録できない）' do
      ...
      @user.password_confirmation = @user.password
      @user.valid?
      expect(@user.errors.full_messages).to include('Password can consist of only half-width characters and must contain at least 1 letter and 1 numeric character')    end
